...
Currently, it is possible to create a multisig wallet using GoldBrix only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/GOLDBRIX-GBX/goldbrix-core/blob/master/doc/descriptors.md#multisig), this tutorial proposes to use the signet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the goldbrix node on the signet network.

```bash
./build/bin/goldbrixd -signet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in GoldBrix. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/goldbrix-cli -signet createwallet "participant_${n}"
done
```

`goldbrix rpc` can also be substituted for `goldbrix-cli`.

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, GoldBrix single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in GoldBrix will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/goldbrix-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/goldbrix-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./build/bin/goldbrix-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/goldbrix-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
`external_desc` and `internal_desc` specify the output type (`wsh`, in this case) and the xpubs involved. They also use BIP 67 (`sortedmulti`), so the wallet can be recreated without worrying about the order of xpubs. Conceptually, descriptors describe a list of scriptPubKey (along with information for spending from it) [[source](https://github.com/GOLDBRIX-GBX/goldbrix-core/issues/21199#issuecomment-780772418)].

Note that at least two descriptors are usually used, one for internal derivation paths and one for external ones. There are discussions about eliminating this redundancy, as can be seen in the issue [#17190](https://github.com/GOLDBRIX-GBX/goldbrix-core/issues/17190).
...
Documentation for these and other parameters can be found by typing `./build/bin/goldbrix-cli help importdescriptors`.
...
```bash
./build/bin/goldbrix-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/goldbrix-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/goldbrix-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/goldbrix-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./build/bin/goldbrix-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/goldbrix-cli -a $receiving_address
```
...
```bash
./build/bin/goldbrix-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./build/bin/goldbrix-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/goldbrix-cli -signet -rpcwallet="participant_1" getnewaddress)
...
```bash
./build/bin/goldbrix-cli -signet decodepsbt $funded_psbt

./build/bin/goldbrix-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/goldbrix-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/goldbrix-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/goldbrix-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/goldbrix-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/goldbrix-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/goldbrix-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/goldbrix-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/goldbrix-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
