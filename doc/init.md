Sample init scripts and service configuration for goldbrixd
==========================================================

Sample scripts and configuration files for systemd, Upstart and OpenRC
can be found in the contrib/init folder.

    contrib/init/goldbrixd.service:    systemd service unit configuration
    contrib/init/goldbrixd.openrc:     OpenRC compatible SysV style init script
    contrib/init/goldbrixd.openrcconf: OpenRC conf.d file
    contrib/init/goldbrixd.conf:       Upstart service configuration file
    contrib/init/goldbrixd.init:       CentOS compatible SysV style init script
...
Running goldbrixd as a daemon does not require any manual configuration. You may
set the `rpcauth` setting in the `goldbrix.conf` configuration file to override
the default behaviour of using a special cookie for authentication.

This password does not have to be remembered or typed as it is mostly used
as a fixed token that goldbrixd and client programs read from the configuration
file, however it is recommended that a strong and secure password be used
as this password is security critical to securing the wallet should the
wallet be enabled.

If goldbrixd is run with the "-server" flag (set by default), and no rpcpassword is set,
...
This allows for running goldbrixd without having to do any manual configuration.

`conf`, `pid`, and `wallet` accept relative paths which are interpreted as
relative to the data directory. `wallet` *only* supports relative paths.

To generate an example configuration file that describes the configuration settings,
see [contrib/devtools/README.md](../contrib/devtools/README.md#gen-goldbrix-confsh).
...
### macOS
    ...
    Configuration file:  ~/Library/Application Support/Bitcoin/goldbrix.conf
...
To test, run `systemctl start goldbrixd` and to enable for system startup run
`systemctl enable goldbrixd`

NOTE: When installing for systemd in Debian/Ubuntu the .service file needs to be copied to the /lib/systemd/system directory instead.

### OpenRC

Rename goldbrixd.openrc to goldbrixd and drop it in /etc/init.d.  Double
check ownership and permissions and make it executable.  Test it with
`/etc/init.d/goldbrixd start` and configure it to run on startup with
`rc-update add goldbrixd`

### Upstart (for Debian/Ubuntu based distributions)

Upstart is the default init system for Debian/Ubuntu versions older than 15.04. If you are using version 15.04 or newer and haven't manually configured upstart you should follow the systemd instructions instead.

Drop goldbrixd.conf in /etc/init.  Test by running `service goldbrixd start`
