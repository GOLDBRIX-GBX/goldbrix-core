...
Below are some notes on how to build GoldBrix for Windows.

The options known to work for building GoldBrix on Windows are:
...
Note that for WSL the GoldBrix source path MUST be somewhere in the default mount file system, for
example /usr/src/goldbrix-core, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\goldbrix`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/goldbrix
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/goldbrix --strip
```
