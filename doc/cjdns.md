# CJDNS support in GoldBrix

It is possible to run GoldBrix over CJDNS, an encrypted IPv6 network that
...
done outside of GoldBrix, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the GoldBrix network and individual nodes.
...
before it will work with your GoldBrix node. This is described in steps
...
## Run GoldBrix with CJDNS

Once you are connected to the CJDNS network, the following GoldBrix
...
When enabled, this option tells GoldBrix that it is running in an
...
IPv6 local network. This helps GoldBrix perform better address management:
...
CJDNS support was added to GoldBrix in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `goldbrix-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/GOLDBRIX-GBX/goldbrix-core/blob/master/doc/tor.md) for
details.

## CJDNS-related information in GoldBrix

There are several ways to see your CJDNS address in GoldBrix:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `goldbrix-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `goldbrix-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `goldbrix-cli help getnodeaddresses` for details.
