...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<goldbrix-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<goldbrix-cli args>]`

* `--cmd` lets you customize the goldbrix-cli path. By default it will look for it in the PATH
* `--faucet` lets you specify which faucet to use; the faucet is assumed to be compatible with the common open-source signet faucet
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `goldbrix-cli` if you use `--addr`)
