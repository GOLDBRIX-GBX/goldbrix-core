# bash programmable completion for goldbrix-cli(1)
# Copyright (c) 2012-2022 The GoldBrix developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $goldbrix-cli for RPC
_goldbrix_rpc() {
...
_goldbrix_cli() {
    ...
    # save and use original argument to invoke goldbrix-cli for -help, help and RPC
    # as goldbrix-cli might not be in $PATH
    goldbrix_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($goldbrix_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_goldbrix_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _goldbrix_cli goldbrix-cli
