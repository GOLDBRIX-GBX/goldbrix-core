...
be used in GoldBrix Core with the `-asmap` option.
...
to binary before being used with GoldBrix Core.
...
the `goldbrix-cli getnodeaddresses` command.
...
```
goldbrix-cli getnodeaddresses 0 > addrs.json
```
