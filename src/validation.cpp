...
{
    // === V31 INSTANT HARD FORK ===
    // Activation at consensusParams.nV31ForkHeight (mainnet=20000, regtest=100)
    // Pre-fork:  50 GBX/block, halving every nSubsidyHalvingInterval
    // Post-fork: nV31InitialSubsidy/block, halving every nV31HalvingInterval
    if (consensusParams.nV31ForkHeight > 0 && nHeight >= consensusParams.nV31ForkHeight) {
        int post_fork_blocks = nHeight - consensusParams.nV31ForkHeight;
        int halvings = post_fork_blocks / consensusParams.nV31HalvingInterval;
        if (halvings >= 64) return 0;
        CAmount nSubsidy = consensusParams.nV31InitialSubsidy;
        nSubsidy >>= halvings;
        return nSubsidy;
    }

    // Pre-fork legacy (original Bitcoin halving math)
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // GBX: witness malleation enforced only from V31 fork; legacy pre-V31 blocks already in chain.
    if (pindexPrev != nullptr && (pindexPrev->nHeight + 1) >= chainman.GetConsensus().nV31ForkHeight) {
...
bool Chainstate::NeedsRedownload() const
{
    ...
    while (block != nullptr && DeploymentActiveAt(*block, m_chainman, Consensus::DEPLOYMENT_SEGWIT)) {
        if (block->nHeight < m_chainman.GetConsensus().nV31ForkHeight) break; // GBX: legacy pre-V31 blocks exempt
